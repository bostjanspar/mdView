...
- [ ] 10.10 Verify the Acceptance list below in full, including `## Title` copying as raw source and blank lines preserved across a multi-block selection
...
- [ ] `Ctrl+Shift+C` over raw lines 12–13 produces: the reference line, then `---`, then exactly lines 12
      and 13 of the file, then `---`.
- [ ] The copied content matches the file byte-for-byte over that range (diff a paste against the source
      lines).
- [ ] A rendered-pane selection over a heading written `## Title` copies `## Title`, not `Title`.
- [ ] A rendered-pane selection spanning several blocks copies every raw line in the unioned range,
      including blank lines between the blocks.
- [ ] A single-line selection produces the `:12-12` reference form followed by that one line.
- [ ] The reference line in the with-content form is identical to what step 09's `Ctrl+C` produces for the
      same selection.
- [ ] A range extending past the last line clamps to the last line and does not raise.
- [ ] The context menu on the document view offers both "Copy Reference" and "Copy Reference with
      Content", and each produces the same result as its shortcut.
- [ ] With no file open, both context entries are unavailable and neither throws.
- [ ] `Ctrl+Shift+C` in a plain text input does not produce a reference.
- [ ] Exactly one code path writes to the clipboard (grep to confirm).
- [ ] `pyright mdview` reports zero errors.
- [ ] Steps 01–09 acceptance lists still pass.
