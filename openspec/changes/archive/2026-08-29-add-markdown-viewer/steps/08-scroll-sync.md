...
- [ ] 8.12 Verify the Acceptance list below in full, including no console errors at either scroll extreme
...
- [ ] Scrolling the rendered pane so the block starting at source line 120 is at the top scrolls the raw
      pane so line 120 is at or near the top (within one block).
- [ ] Scrolling the raw pane so line 120 is at the top scrolls the rendered pane to the block containing
      or most closely preceding line 120.
- [ ] With the raw pane's top line in the middle of a 50-line fenced block, the rendered pane shows that
      code block aligned to its start.
- [ ] One scroll gesture settles both panes at one final position each — no oscillation, no continuous
      drift, no visible jitter.
- [ ] Fast repeated scrolling in one pane never causes the panes to fight each other.
- [ ] Scrolling in Raw mode and in Rendered mode has no sync side effects.
- [ ] Leaving Both mode and returning re-aligns the panes and sync still works.
- [ ] Editing the file externally in Both mode and then scrolling either pane syncs correctly against
      the updated line numbers.
- [ ] Sync still works after switching to a different file from the tree while in Both mode.
- [ ] Scrolling to the very top and the very bottom of either pane does not throw a console error.
- [ ] No console errors appear during any of the above.
- [ ] Steps 01–07 acceptance lists still pass.
