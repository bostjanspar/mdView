...
- [ ] 9.10 Verify the Acceptance list below in full, including backwards selections and the no-file-open case
...
- [ ] Selecting raw lines 12–18 and pressing `Ctrl+C` puts exactly `C:/…/notes.md:12-18` on the
      clipboard — forward slashes, absolute path, no quotes, no trailing newline beyond one.
- [ ] A selection entirely within line 12 yields `…:12-12`, never `…:12`.
- [ ] A selection dragged upward from line 18 to line 12 yields `12-18`, the same as dragging downward.
- [ ] Selecting rendered text that came from source lines 30–34 yields `30-34`.
- [ ] A rendered selection starting in a block mapped to 10–12 and ending in a block mapped to 20–22
      yields `10-22`.
- [ ] Selecting rendered heading text produced by `## Title` yields the heading's source range, not a
      guess based on the visible text.
- [ ] Both panes work in Both mode; the pane that holds the selection is the one that determines the
      range.
- [ ] With focus in a plain text input (add a temporary input to confirm, then keep or remove it as the
      UI requires), `Ctrl+C` copies that field's selected text and produces no reference.
- [ ] With the document view focused but nothing selected, `Ctrl+C` writes nothing to the clipboard — the
      previous clipboard contents survive.
- [ ] With no file open, `Ctrl+C` writes nothing and does not throw.
- [ ] The reported line numbers are raw source lines in every case, verified by cross-checking against
      the raw pane's gutter.
- [ ] `build_reference` returns a `Reference`, not a tuple or a formatted string.
- [ ] `pyright mdview` reports zero errors.
- [ ] Steps 01–08 acceptance lists still pass.
